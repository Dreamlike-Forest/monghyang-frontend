...
const UserSignupForm: React.FC<UserSignupFormProps> = ({ onBack }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.is_agreed) newErrors.is_agreed = '개인정보 약관에 동의해주세요.';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('일반 사용자 회원가입:', formData);
    // 실제 회원가입 API 호출
  };
          ...
          <div className="user-form-group">
            <div className="user-checkbox-group">
              <input
                ...
                onChange={handleInputChange}
              />
              <label htmlFor="is_agreed" className="user-checkbox-label">
                개인정보 수집 및 이용에 동의합니다 *
              </label>
            </div>
            {errors.is_agreed && <span className="user-error-message">{errors.is_agreed}</span>}
          </div>
