...
const BreweryImageGallery: React.FC<BreweryImageGalleryProps> = ({ brewery, forwardRef }) => {
  ...
  const getBreweryImages = (): string[] => {
    ...
    // 추가 이미지들 (brewery 객체에 확장 필드가 있는 경우)
    const breweryAny = brewery as any;
    if (breweryAny.brewery_image_image_key && breweryAny.brewery_image_image_key.length > 0) {
      breweryAny.brewery_image_image_key.forEach((imageObj: any) => {
