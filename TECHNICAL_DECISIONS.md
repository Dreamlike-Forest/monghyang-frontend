# 몽향(Monghyang) 플랫폼 기술 의사결정 및 아키텍처 분석 보고서 (Technical Decisions)

본 문서는 전통주 이커머스 및 양조장 탐방·체험 예약 통합 플랫폼 **'몽향(Monghyang)'**의 프론트엔드-백엔드 연동 및 시스템 구축 과정에서 수립된 **기술적 의사결정(Technical Decisions), 대안 기술 비교(Alternatives Considered), 그리고 실제 문제 해결(Troubleshooting) 사례**를 정리한 기술 아키텍처 문서입니다.

---

## 1. 프로젝트 개요

- **프로젝트명**: 몽향 (Monghyang Platform)
- **도메인 영역**:
  1. **전통주 커머스**: 주종별/도수별 필터링, 장바구니, 2단계 PG 결제 트랜잭션, 주문 내역 및 취소
  2. **양조장 탐방 & 체험 예약**: 양조장 상세 정보, 실시간 캘린더 기반 체험 예약(`joy-order`), 타임슬롯별 정원 관리
  3. **전통주 애호가 커뮤니티**: 주종/양조장 리뷰, 이미지 다중 업로드, 대댓글 및 좋아요
  4. **다중 권한(RBAC) 회원 시스템**: 일반 유저, 전통주 판매자, 양조장 사업자, 총괄 관리자
- **개발 단계**: 프론트엔드-백엔드 실서버 연동 및 결제/예약/커뮤니티 파이프라인 완성 단계

---

## 2. 전체 시스템 아키텍처 및 기술 스택 현황
...
| **Icons & UI** | Lucide React + Vanilla CSS (CSS Modules) | 순수 CSS 스타일링으로 번들 다이어트 및 일관된 한국 전통 무드 구현 |
| **Backend Integration** | Spring Boot (REST API, Redis Session, S3) | Multipart FormData, Custom Header Session, Spring PageResponse 정규화 |
...
graph TD
    subgraph "Client Tier (Next.js 14 / TypeScript)"
        ...
        CS["CartStore (Custom Observer Pattern)"]
...
## 3. 핵심 기술적 의사결정 및 대안 비교 분석 (Why & What)

### Decision 1. 프론트엔드 프레임워크 및 라우팅 아키텍처

#### [배경 및 요구사항]
- 이커머스 및 양조장 정보 특성상 검색엔진 최적화(SEO) 및 이미지/폰트 로딩 최적화 기반이 장기적으로 요구됨.
- 반면 개발 및 연동 단계에서는 화면 전환 시 글로벌 상태(장바구니 수량, 헤더 로그인 상태, 모달 레이어)가 초기화되지 않고 네이티브 앱처럼 부드럽게 유지되어야 함.

#### [고려했던 대안]
1. **순수 React SPA (Vite / CRA)**
   - *장점*: 라우팅 및 번들 설정이 단순하며 클라이언트 상태 유지가 쉬움.
   - *단점*: 향후 SSR/SSG 확장 불가, 이미지 최적화 및 메타태그 관리 부재, 빌드 최적화 한계.
2. **Next.js 14 App Router의 완전한 다중 페이지 파일 라우팅 (`app/shop/page.tsx`, `app/brewery/page.tsx` 등)**
   - *장점*: Next.js 표준 디렉토리 라우팅 구조 준수.
   - *단점*: 백엔드 세션(`localStorage` 기반) 의존 컴포넌트들의 Hydration Mismatch 빈발, 잦은 페이지 전환 시 레이아웃 깜빡임 및 비동기 상태 재패칭 비용 발생.
3. **[최종 선택] Next.js 14 App Router + Query Parameter 기반 클라이언트 동적 뷰 스위칭 (`MainApp.tsx`)**
   - *채택 이유*:
     ...
     - 컴포넌트 모듈화가 잘 되어 있어 추후 언제든 개별 독립 라우트로 분리 확장이 용이함.

---

### Decision 2. 상태 관리 아키텍처: Custom Observer Pattern vs 외부 라이브러리

#### [배경 및 요구사항]
- 장바구니 수량(Cart Badge)은 상단 헤더, 네비게이션, 상품 목록, 상세 페이지, 장바구니 뷰 등 서비스 전역에서 실시간 동기화되어야 함.
- 백엔드 장바구니 API(`/api/cart`)와 로컬 UI 상태를 실시간으로 맞추어야 하며, 불필요한 의존성 설치로 번들 사이즈가 커지는 것을 방지해야 함.

#### [고려했던 대안]
1. **Redux Toolkit (RTK)**
   - *검토 결과*: 장바구니 및 전역 데이터 하나를 위해 Slice, Thunk, Provider 등 작성해야 할 보일러플레이트 코드가 너무 방대함.
2. **Zustand / Recoil**
   - *검토 결과*: 경량 상태 라이브러리이나 외부 패키지 의존성이 추가됨.
3. **React Context API**
   - *검토 결과*: Context 값이 갱신될 때 Provider 하위의 불필요한 전체 컴포넌트 트리가 리렌더링되는 성능 저하(Re-render Waterfall) 발생.
4. **[최종 선택] 순수 TypeScript 기반 경량 옵저버 패턴(Pub/Sub) 인메모리 스토어 (`CartStore.ts`) 직접 구현**
   - *채택 이유*:
     - **Zero Dependency**: 외부 라이브러리를 일절 추가하지 않아 번들 크기 0KB 추가.
     - **프레임워크 비종속성**: React 컴포넌트 수명주기 외부(일반 유틸 함수, Axios 인터셉터 등)에서도 언제든 상태 접근 및 구독 가능.
     - **정밀 리렌더링**: 구독자 목록(`cartListeners: (() => void)[]`)에 등록된 컴포넌트만 `notifyCartChange()` 시점에 선별적으로 리렌더링되어 최적의 렌더링 성능 달성.
...
### Decision 3. 인증 및 세션 보안 아키텍처: 커스텀 헤더 기반 하이브리드 세션 관리

#### [배경 및 요구사항]
- 백엔드가 Redis 기반의 분산 세션 스토리지를 운용하고 있으며, 다중 기기 로그인 통제(동시 접속 제한)와 즉각적인 세션 만료 제어가 필수적이었음.
- 프론트엔드와 백엔드가 서로 다른 도메인/포트 환경(Cross-Origin)에서 구동될 때 발생할 수 있는 브라우저 쿠키 제약(SameSite, Safari ITP 등)을 극복해야 함.

#### [고려했던 대안]
1. **표준 JWT Bearer 토큰 (Local Storage 저장)**
   - *한계*: JWT 자체는 Stateless하여, 다른 기기에서 로그인했을 때 기존 기기를 강제 로그아웃시키는 동시 세션 제어가 불가능함.
2. **순수 HttpOnly 쿠키 기반 세션**
   - *한계*: 개발/스테이징 환경의 서로 다른 서브도메인 간 쿠키 공유 이슈 및 모바일 웹뷰 연동 시 쿠키 유실 위험, CSRF 공격 대비 토큰 필요.
3. **[최종 선택] Custom Header (`X-Session-Id` + `X-Refresh-Token`) + Axios Interceptor 2중 안전장치**
   - *채택 이유*:
     - **동시 로그인 통제 (409 Conflict)**: 다른 기기 로그인 발생 시 서버가 409 상태코드를 반환하고, 클라이언트는 즉시 이를 감지하여 사용자에게 알린 후 세션을 파기함.
...
```typescript
// utils/api.ts: 401 시 자동 재시도 및 409 동시접속 제어
...
- **계층형 가중치 모델 (`ROLE_HIERARCHY`)**:
  ...
  ```
- **고차 권한 함수 (`withPermission`) & 커스텀 확인 모달 (`showCustomConfirm`)**:
  - 권한이 부족한 사용자가 접근 시 브라우저 기본 팝업 대신 세련된 글래스모피즘(Glassmorphism) 커스텀 모달로 안내.
  - 컴포넌트 코드의 중복 `if`문을 제거하고 선언적으로 권한을 검증함.

---

### Decision 5. 대용량 미디어 처리 및 클라이언트 사이드 리사이징 파이프라인
...
#### [고려했던 대안]
1. **서버 사이드 전량 리사이징 (Spring Boot + AWS Lambda)**
   - *단점*: 서버 전송 시간 자체는 줄어들지 않으며, 대용량 파일 전송 중 네트워크 타임아웃 발생 위험.
2. **외부 유료 이미지 CDN / SaaS (Cloudinary 등)**
   - *단점*: 추가 비용 발생 및 서드파티 의존도 증가.
3. **[최종 선택] HTML5 Canvas API를 활용한 클라이언트 사전 리사이징 (`resizeImage`)**
   - *채택 이유*:
     ...
     - `URL.createObjectURL`로 즉각적인 이미지 프리뷰를 띄우고, 작업 완료 시 `revokeObjectURL`을 호출하여 브라우저 메모리 누수를 완벽히 방지함.

---

### Decision 6. 이커머스 주문 및 체험 예약 결제 트랜잭션 (2-Phase Verification)

#### [배경 및 요구사항]
- 결제 프로세스에서 클라이언트 측의 악의적인 금액 변조, 재고 부족 상태에서의 결제 시도, 네트워크 단절로 인한 결제 승인 누락 등의 치명적인 결제 사고를 방지해야 함.

#### [결제 아키텍처: 2단계 검증(Prepare & Approve)]
1. **1단계 - 주문 준비 (`/api/orders/prepare` or `/api/joy-order/prepare`)**:
   - 클라이언트는 장바구니 상품 ID 목록과 배송지 정보를 서버로 전송.
   - 서버는 데이터베이스 상의 실제 상품 가격과 재고를 조회하여 총 금액을 계산하고, 고유 주문 식별자(`pgOrderId`)를 발급. (클라이언트가 보낸 금액을 신뢰하지 않음)
...
### Decision 7. 백엔드(Spring Boot) 데이터 규격과의 정규화 어댑터 패턴

#### [배경 및 요구사항]
- 백엔드(Spring Data JPA)의 페이징 응답 객체(`Page<T>`)는 복잡한 중첩 구조(`pageable`, `sort`, `numberOfElements`, `first`, `last` 등)를 지님.
- 프론트엔드 컴포넌트마다 다른 페이징 프로퍼티(`page_number` vs `pageNumber`, `total_pages` vs `totalPages`)로 인한 undefined 참조 에러 방지 필요.

#### [해결책: Data Normalization Adapter]
- `shopApi.ts`, `communityApi.ts` 등에 `normalizePageResponse<T>` 및 `createEmptyPageResponse<T>` 함수를 구축.
- 백엔드의 스네이크 케이스(`page_size`)와 카멜 케이스(`pageSize`)를 모두 포용하는 런타임 방어 코드를 작성하여 백엔드 API 응답 구조 변경에도 프론트엔드가 깨지지 않도록 견고성 확보.

---

## 4. 백엔드 협업 중 발생한 주요 트러블슈팅 및 해결 사례
